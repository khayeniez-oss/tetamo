...
export default function EditPageClient() {
  ...
  const [listingCode, setListingCode] = useState(draft?.kode ?? kode ?? "");
  ...
  useEffect(() => {
    ...
    setListingCode(draft?.kode ?? kode ?? "");
  ...
  const finalProvinces = useMemo(() => uniqueStrings(DEFAULT_PROVINCES), []);
  ...
  const canNext = useMemo(() => {
    return (
      ...
      listingCode.trim().length > 0 &&
      postedDate.trim().length > 0 &&
      address.trim().length > 0 &&
      province.trim().length > 0 &&
      city.trim().length > 0
    );
  }, [listingType, vehicleType, listingCode, postedDate, address, province, city]);
  ...
  function handleNext() {
    ...
    router.push(`/vehicles/edit/${encodeURIComponent(listingCode)}/detail?plan=${currentPlan}`);
  }

  function handleBack() {
    router.push(getMarketplaceHref(vehicleType || draft?.vehicleType || ""));
  }
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-xl font-bold leading-tight tracking-tight text-[#1C1C1E] sm:text-2xl lg:text-3xl">
                {lang === "id" ? "Edit Lokasi Kendaraan" : "Edit Vehicle Location"}
              </h1>
            ...
            <p className="mt-2 text-sm leading-6 text-gray-600">
              {lang === "id"
                ? "(Step 1) Edit jenis listing, pilih car atau motor, lalu perbarui lokasi kendaraan."
                : "(Step 1) Edit the listing type, choose car or motor, then update the vehicle location."}
            </p>
          </div>
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  list="vehicle-edit-city-area-suggestions"
                  type="text"
                  value={city}
                ...
                <datalist id="vehicle-edit-city-area-suggestions">
