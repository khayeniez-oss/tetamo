...
export default function MediaPageClient() {
  ...
  const vehicleType = initial.vehicleType === "motor" ? "motor" : "car";
  ...
  const t =
    lang === "id"
      ? {
          ...
          pageTitle:
            vehicleType === "motor"
              ? "Edit Foto & Video Motor"
              : "Edit Foto & Video Mobil",
          pageSubtitle:
            vehicleType === "motor"
              ? "Perbarui foto, video, judul, dan deskripsi motor sebelum dikirim untuk approval."
              : "Perbarui foto, video, judul, dan deskripsi mobil sebelum dikirim untuk approval.",
          addPhotos: "Tambah Foto",
          uploading: "Mengupload...",
          removeAll: "Hapus semua",
          noPhotosYet: "Belum ada foto.",
          listingTitle:
            vehicleType === "motor" ? "Judul Motor" : "Judul Mobil",
          ...
          titleRequired:
            vehicleType === "motor"
              ? "Judul motor wajib diisi."
              : "Judul mobil wajib diisi.",
          ...
          descGuide3: "Jelaskan poin penting kendaraan dengan jelas.",
          uploadPhotoHint:
      ...
      : {
          ...
          pageTitle:
            vehicleType === "motor"
              ? "Edit Motorbike Photos & Video"
              : "Edit Car Photos & Video",
          pageSubtitle:
            vehicleType === "motor"
              ? "Update the motorbike photos, video, title, and description before submitting for approval."
              : "Update the car photos, video, title, and description before submitting for approval.",
          addPhotos: "Add Photos",
          uploading: "Uploading...",
          removeAll: "Remove all",
          noPhotosYet: "No photos yet.",
          listingTitle:
            vehicleType === "motor" ? "Motorbike Title" : "Car Title",
          ...
          titleRequired:
            vehicleType === "motor"
              ? "Motorbike title is required."
              : "Car title is required.",
          ...
          descGuide3: "Explain the key vehicle points clearly.",
          uploadPhotoHint:
  ...
  const photoInputId = "vehicle-edit-photo-input";
  const videoInputId = "vehicle-edit-video-input";
  ...
  async function ensureUserAndFolder() {
    ...
    const folder =
      draft?.mediaFolder || draft?.kode || kode || crypto.randomUUID();
  ...
  async function onAddVideo(files: FileList | null) {
    ...
    try {
      ...
      if (video) {
        const oldPath = getPublicFilePathFromUrl(video, "vehicle-videos");
        if (oldPath) {
          await supabase.storage.from("vehicle-videos").remove([oldPath]);
        }
      }
  ...
  async function removePhoto(idx: number) {
    ...
    const path = getPublicFilePathFromUrl(targetUrl, "vehicle-images");
  ...
  async function clearAllPhotos() {
    const paths = photos
      .map((url) => getPublicFilePathFromUrl(url, "vehicle-images"))
  ...
  async function clearVideo() {
    const path = getPublicFilePathFromUrl(video, "vehicle-videos");
    setVideo("");

    if (!path) return;

    try {
      await supabase.storage.from("vehicle-videos").remove([path]);
    } catch (error) {
  ...
  function handleBack() {
    router.push(
      `/vehicles/edit/${encodeURIComponent(listingCode)}/detail?plan=${currentPlan}`
    );
  }
  ...
  function handleNext() {
    ...
    setDraft((prev: any) => ({
      ...
      verifiedOk: true,
      editSubmissionStatus: "pending_approval",
    }));

    router.push(
      `/vehicles/edit/${encodeURIComponent(
        listingCode
      )}/success?status=pending-approval&vehicleType=${vehicleType}&kode=${encodeURIComponent(
        listingCode
      )}`
    );
  }
          ...
          <div className="p-4 sm:p-5 md:p-6 lg:p-7">
            ...
            <div className="mt-8 border-t border-gray-100 pt-8 sm:mt-10 sm:pt-10">
              <div className="flex items-center justify-between gap-3 text-sm">
                <label className="font-semibold">{t.listingTitle} *</label>
