...
function mapVehicleToCar(
  ...
  return {
    ...
    images: images.length > 0 ? images : [FALLBACK_CAR_IMAGE],
    postedByType,
...
async function fetchCarsFromSupabase(lang: "id" | "en") {
  ...
  return baseRows.map((row) => mapVehicleToCar(row, mediaMap, profileMap, lang));
}
  ...
  function renderVerificationBadge() {
    if (p.postedByType === "dealer") {
      ...
      if (p.dealerPendingVerification) {
        return (
          <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-[10px] font-semibold text-amber-700 shadow-sm sm:text-[11px]">
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="mt-1 text-sm text-gray-500">≈ {secondaryPrice}</div>
        ...
        <div className="mt-2 flex flex-wrap gap-x-2 gap-y-1 text-[12px] tracking-wide text-gray-500">
          <span>{p.kode}</span>
          <span>•</span>
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          ...
          >
            <div className="text-sm font-bold text-[#1C1C1E]">{shareCount}</div>
...
export default function CarPageClient() {
  ...
  const bodyType = normalizeText(sp.get("type") || "");
  ...
  useEffect(() => {
    ...
    async function loadCars() {
      try {
        setLoading(true);
        const rows = await fetchCarsFromSupabase(lang);
        if (ignore) return;
        setAll(rows);
      } catch (error) {
  ...
  function handleToggleSave(propertyId: string) {
    ...
    setSaveCountMap((prev) => ({
      ...prev,
      [propertyId]: Math.max(0, (prev[propertyId] ?? 0) + (currentlySaved ? -1 : 1)),
    }));
  }
  ...
  function handleToggleLike(propertyId: string) {
    ...
    setLikeCountMap((prev) => ({
      ...prev,
      [propertyId]: Math.max(0, (prev[propertyId] ?? 0) + (currentlyLiked ? -1 : 1)),
    }));
  }
  ...
  function handleRate(propertyId: string, rating: number) {
    ...
    const nextUserRating = currentUserRating === rating ? 0 : rating;

    let nextCount = currentSummary.count;
    let total = currentSummary.avg * currentSummary.count;
    ...
    if (nextUserRating > 0) {
      total += nextUserRating;
      nextCount += 1;
    }

    const nextAvg = nextCount > 0 ? total / nextCount : 0;

    setUserRatingsMap((prev) => ({
      ...prev,
      [propertyId]: nextUserRating,
    }));
  ...
  async function handleShare(property: CarListing) {
    const shareUrl =
      typeof window !== "undefined"
        ? `${window.location.origin}/vehicles/car/${property.slug || property.id}`
        : `/vehicles/car/${property.slug || property.id}`;
  ...
  const filtered = useMemo(() => {
    ...
    if (bodyType) {
      next = next.filter((item) => normalizeText(item.bodyType) === bodyType);
    }

    const q = normalizeText(marketplaceSearch);
    if (q) {
      next = next.filter((item) =>
        normalizeText(
          `${item.title} ${item.area} ${item.province} ${item.kode} ${item.sellerName} ${item.bodyType} ${item.fuel} ${item.transmission}`
        ).includes(q)
      );
    }

    return next.sort(sortByRanking);
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <div className="rounded-[32px] bg-[#F7F7F8] p-5 sm:p-8 lg:p-10">
          <div className="max-w-4xl">
            <h1 className="text-[30px] font-bold leading-[1.08] tracking-[-0.03em] text-[#1C1C1E] sm:text-[40px] lg:text-[52px]">
              {lang === "id" ? "Marketplace Mobil" : "Car Marketplace"}
            </h1>

            <p className="mt-4 max-w-3xl text-sm leading-7 text-gray-600 sm:text-base">
              {lang === "id"
                ? "Jelajahi listing mobil dengan tampilan yang lebih rapi, filter yang jelas, dan tombol tindakan yang siap digunakan."
                : "Browse car listings with a cleaner presentation, clearer filters, and action buttons ready to use."}
            </p>

            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_120px]">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={
                    lang === "id"
                      ? "Cari brand, model, lokasi, kode..."
                      : "Search brand, model, location, code..."
                  }
                  className="h-12 w-full rounded-2xl border border-gray-200 bg-white pl-11 pr-4 text-sm outline-none focus:border-[#1C1C1E]"
                />
              </div>

              <button
                type="button"
                className="h-12 rounded-2xl bg-[#1C1C1E] px-4 text-sm font-semibold text-white transition hover:opacity-90"
                onClick={() => setPage(1)}
              >
                {lang === "id" ? "Cari" : "Search"}
              </button>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              />
              <FilterChip href="/vehicles/car?type=suv" active={bodyType === "suv"} label="SUV" />
              <FilterChip href="/vehicles/car?type=sedan" active={bodyType === "sedan"} label="Sedan" />
              <FilterChip
                href="/vehicles/car?type=ev"
                active={bodyType === "ev"}
                label={lang === "id" ? "Listrik" : "EV"}
              />
              <FilterChip href="/vehicles/car?type=mpv" active={bodyType === "mpv"} label="MPV" />
              <FilterChip
                href="/vehicles/car?type=sport"
                active={bodyType === "sport"}
                label={lang === "id" ? "Sport" : "Sport"}
              />
            </div>
