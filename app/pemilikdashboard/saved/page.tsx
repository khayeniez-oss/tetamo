...
export default function PemilikSavedPage() {
  ...
  const t =
    lang === "id"
      ? {
          ...
          pageSubtitle:
            "Semua properti yang Anda simpan akan muncul di sini.",
  ...
  useEffect(() => {
    ...
    async function loadSaved() {
      ...
      if (!user) {
        ...
        setItems([]);
        setLoading(false);
        router.push("/login?next=/pemilikdashboard/saved");
        return;
      }
      ...
      if (savedError) {
        console.error("Failed to load saved properties:", savedError);
        setItems([]);
        setLoading(false);
        return;
      }
      ...
      if (propertyError) {
        console.error("Failed to load property data for saved items:", propertyError);
        setItems([]);
        setLoading(false);
        return;
      }
  ...
  const totalSaved = useMemo(() => items.length, [items.length]);
