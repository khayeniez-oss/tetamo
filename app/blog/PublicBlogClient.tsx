...
import { Bookmark, Eye, Heart, Search, Share2 } from "lucide-react";
...
export default function PublicBlogClient({
  ...
  }, [initialBlogs, searchQuery, lang]);

  function toggleSave(blogId: string) {
    setSavedBlogs((prev) => {
      const nextValue = !prev[blogId];

      localStorage.setItem(`tetamo_blog_saved_${blogId}`, String(nextValue));
  ...
  function toggleLike(blogId: string) {
    setLikedBlogs((prev) => {
      ...
      localStorage.setItem(`tetamo_blog_liked_${blogId}`, String(nextValue));
