...
function contentToLines(raw?: string | null, blogTitle?: string | null) {
  ...
  if (hasHtml) {
    const flattened = decoded
      .replace(/<\s*br\s*\/?>/gi, "\n")
      .replace(/<\/(p|h1|h2|h3|h4|li|blockquote|div|section|article)>/gi, "\n")
      .replace(/<li[^>]*>/gi, "• ")
...
export default function PublicBlogDetailClient({
  initialBlog,
  initialBlocks,
}: {
  ...
  const [blocks] = useState<BlogContentBlock[]>(initialBlocks);
  ...
  useEffect(() => {
    ...
    const savedKey = `tetamo_blog_saved_${blog.id}`;
    const likedKey = `tetamo_blog_liked_${blog.id}`;
