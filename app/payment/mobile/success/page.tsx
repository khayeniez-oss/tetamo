...
function buildDeepLink(params: URLSearchParams) {
  ...
  const deepLink = new URL(
    role === "agent"
      ? "tetamomobile://agent/payment-success"
      : "tetamomobile://owner/payment-success"
  );

  deepLink.searchParams.set("payment", "success");
...
export default function MobilePaymentSuccessPage() {
  ...
  const { deepLink, role, paymentId } = useMemo(() => {
    if (!currentUrl) {
      return {
        deepLink: "tetamomobile://owner/payment-success?payment=success",
        role: "owner" as Role,
        paymentId: "",
      };
    }
  ...
  return (
    <main style={styles.page}>
      <section style={styles.card}>
        ...
        <p style={styles.kicker}>PAYMENT RECEIVED</p>

        <h1 style={styles.title}>
          {role === "agent"
            ? "Agent Package Payment Successful"
            : "Listing Payment Successful"}
        </h1>

        <p style={styles.description}>
          Your payment has been received. Open the Tetamo app to view the
          updated payment status, receipt, and next steps.
        </p>

        <div style={styles.statusBox}>
          <p style={styles.statusLabel}>Payment ID</p>
          <p style={styles.statusValue}>{paymentId || "Processing..."}</p>
        </div>
        ...
        <p style={styles.note}>
          If the app does not open automatically, tap the button above. Your
          payment status is confirmed by Tetamo using the payment webhook.
        </p>
      </section>
    </main>
  );
