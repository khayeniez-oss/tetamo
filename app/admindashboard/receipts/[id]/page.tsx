...
function getReceiptNumber(payment: PaymentTransactionRow | null) {
  ...
  if (payment.stripe_payment_intent_id?.trim()) {
    return `RCT-${payment.stripe_payment_intent_id.trim()}`;
  }

  return `RCT-${payment.id.slice(0, 8).toUpperCase()}`;
}

function getInvoiceNumber(payment: PaymentTransactionRow | null) {
  if (!payment) return "-";

  if (payment.stripe_invoice_id?.trim()) {
    return payment.stripe_invoice_id.trim();
  }

  if (payment.stripe_checkout_session_id?.trim()) {
    return `INV-${payment.stripe_checkout_session_id.slice(0, 8).toUpperCase()}`;
  }
...
export default function AdminReceiptDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadReceipt() {
      ...
      const row = data as Omit<PaymentTransactionRow, "status"> & {
        status: string | null;
      };
  ...
  const meta = asObject(payment.metadata);
  ...
  const location = [meta.property_city, meta.property_province]
    .filter(Boolean)
    .join(", ");
        ...
        <div className="grid gap-4 border-b border-gray-100 px-4 py-5 sm:gap-6 sm:px-6 sm:py-6 md:grid-cols-2 md:px-8 md:py-8">
          <div>
            ...
            <p className="mt-2 text-[12px] text-gray-500 sm:mt-3 sm:text-base">
              {getRoleLabel(payment.source_role)}
            </p>
          </div>
                  ...
                  <div className="rounded-2xl border border-gray-100 bg-gray-50 p-3">
                    ...
                    <p className="mt-1 text-[12px] text-gray-600 sm:text-sm">
                      {payment.property_code_snapshot
                        ? `Listing: ${payment.property_code_snapshot}`
                        : "Listing: -"}
                    </p>
                  </div>
